...
const A_DIGIT: f32 = 1.0;
const B_DIGIT: f32 = 9.0;
const FOV: f32 = 2.0;
...
impl State {
    pub async fn new(window: Arc<Window>, config_zad1: bool) -> anyhow::Result<State> {
        ...
        // Camera
        let aspect = 6.4 / 4.8;
        let fov_y = FOV * (2.4_f32 / 5.0).atan();
        let proj = glam::Mat4::perspective_rh(fov_y, aspect, 5.0, 100.0);
    ...
    pub fn update(&mut self) {
        ...
        let roll_a = (n * (2.0 + A_DIGIT / 10.0)).to_radians();
        let pitch_a = (n * (1.0 + B_DIGIT / 10.0)).to_radians();
        ...
        let roll_b = (-n * (2.0 + B_DIGIT / 10.0)).to_radians();
        let pitch_b = (-n * (1.0 + A_DIGIT / 10.0)).to_radians();
...
fn create_pipeline(
    ...
    let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        label: Some("Shader"),
        source: wgpu::ShaderSource::Wgsl(context.into()),
    });
